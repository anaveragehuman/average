...
SRC_URI="https://github.com/sgothel/gluegen/archive/v${PV}.tar.gz -> ${P}.tar.gz
		 https://github.com/sgothel/jcpp/archive/v${PV}.tar.gz -> ${JCPP_P}.tar.gz
"
...
src_unpack() {
	default
	mv "${S}/../${JCPP_P}/src" "${S}/jcpp/"
}

src_prepare() {
	default
	rm -rf make/lib
	eapply -p2 "${FILESDIR}"/${PV}-*.patch

	# FIXME: linker flags to gcc are not valid for ld; upstream bug?
	sed -i '/-static-/d' make/gluegen-cpptasks-base.xml

	java-ant_bsfix_files make/build-test.xml
}
